...
namespace CarRentalManagement.Server.Repository
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        public async Task Save(HttpContext httpContext)
        {
            ...
            {
                ((BaseDomianModel)entry.Entity).DateUpdate = DateTime.Now;
                ((BaseDomianModel)entry.Entity).UpdateBy = user;
                if (entry.State == EntityState.Added)
                {
                    ((BaseDomianModel)entry.Entity).DateCreated = DateTime.Now;
                    ((BaseDomianModel)entry.Entity).CreatedBy = user;
                }
            }
